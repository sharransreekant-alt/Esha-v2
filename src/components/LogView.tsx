...
import { inputToDate, fmtTime, toDate } from '../utils/helpers'

interface ActionBtn { emoji: string; name: string; sub: string; bg: string; action: () => void }

function SimpleModal({ emoji, title, onClose, onSave, hasDuration, hasNotes = true }: { emoji: string; title: string; onClose: () => void; onSave: (t: string, notes: string, dur?: string) => void; hasDuration?: boolean; hasNotes?: boolean }) {
  const [time, setTime] = useState<string>((() => { const n = new Date(); return `${String(n.getHours()).padStart(2,'0')}:${String(n.getMinutes()).padStart(2,'0')}` })())
  ...
  return (
    <div onClick={onClose} style={{ position: 'fixed', inset: 0, background: 'rgba(46,28,18,0.45)', zIndex: 100, display: 'flex', alignItems: 'flex-end', backdropFilter: 'blur(8px)' }}>
      <div onClick={e => e.stopPropagation()} style={{ background: 'var(--white)', borderRadius: 'var(--r) var(--r) 0 0', width: '100%', maxWidth: 430, margin: '0 auto', padding: '8px 20px 48px', boxShadow: '0 -8px 40px rgba(100,60,20,0.18)' }}>
        ...
        {hasDuration && <div className="fg"><label className="flbl">Duration (minutes)</label><input className="finput" type="number" inputMode="numeric" placeholder="e.g. 10" value={dur} onChange={e => setDur(e.target.value)} /></div>}
        <div className="fg">
          ...
          <div style={{ display: 'flex', gap: 8 }}>
            ...
            <button onClick={() => { const n = new Date(); setTime(`${String(n.getHours()).padStart(2,'0')}:${String(n.getMinutes()).padStart(2,'0')}`) }} style={{ padding: '0 14px', background: 'var(--cream2)', border: '1.5px solid var(--border)', borderRadius: 'var(--r-sm)', color: 'var(--text-med)', fontSize: 13, fontWeight: 700 }}>Now</button>
          </div>
        </div>
        {hasNotes && <div className="fg"><label className="flbl">Notes (optional)</label><input className="finput" type="text" placeholder="Any observations…" value={notes} onChange={e => setNotes(e.target.value)} /></div>}
        <button className="btn-primary" onClick={() => onSave(time, notes, dur)} style={{ marginTop: 6 }}>Save</button>
...
export function LogView() {
  ...
  async function saveSimple(type: string, time: string, notes: string, dur?: string) {
    await saveEntry({ type: type as any, notes: notes || null, duration: dur ? parseInt(dur) : undefined, _t: inputToDate(time) } as any)
    close()
  }
  ...
  async function saveFeed(components: FeedComponent[], time: string, notes: string) {
    ...
    if (editEntry) {
      await updateEntry(editEntry.id, {
        feedType: components[0].feedType, components,
        duration: totalDur || null, volume: totalVol || null,
        notes: notes || null,
        timestamp: inputToDate(time) as any,
      })
    } else {
      await saveEntry({ type: 'feed', feedType: components[0].feedType, components, duration: totalDur || null, volume: totalVol || null, notes: notes || null, _t: inputToDate(time) } as any)
    }
    close()
  }

  const BTN_STYLE = (bg: string): React.CSSProperties => ({
    background: 'var(--white)', border: 'none', borderRadius: 'var(--r)',
    boxShadow: 'var(--shadow)', padding: '16px 13px 14px',
    cursor: 'pointer', display: 'flex', flexDirection: 'column', alignItems: 'flex-start', gap: 10, textAlign: 'left',
  ...
  return (
    <div style={{ padding: '18px 16px 72px' }}>
      ...
      <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: 9, marginBottom: 26 }}>
        {actions.map(a => (
          <button key={a.name} onClick={a.action} style={BTN_STYLE(a.bg)}>
      ...
      {modal === 'feed' && (
        <FeedModal
          onSave={saveFeed} onClose={close} isEdit={!!editEntry}
          initial={editEntry ? { components: editEntry.components || (editEntry.feedType ? [{ feedType: editEntry.feedType, duration: editEntry.duration, volume: editEntry.volume }] : []), time: fmtTime(editEntry.timestamp), notes: editEntry.notes || '' } : undefined}
        />
      )}
